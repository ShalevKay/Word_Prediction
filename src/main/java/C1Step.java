...
public class C1Step {
    ...
    public static class MapperClass extends Mapper<LongWritable, Text, Text, Text> {
        ...
        public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
            ...
            int occurrences = Integer.parseInt(lineParts[1].trim());

            keyOut.set(ngram);
            valueOut.set(String.valueOf(occurrences));
    ...
    public static class PartitionerClass extends Partitioner<Text, Text> {
        ...
        public int getPartition(Text key, Text value, int numPartitions) {
            ...
            String indicator = keyParts.length == 3 ? keyParts[1] : key.toString();
    ...
    public static class ComparatorClass extends WritableComparator {
        ...
        public int compare(WritableComparable w1, WritableComparable w2) {
            ...
            if (t1Parts.length == 1 && t2Parts.length == 3) {
                if (t1Parts[0].equals(t2Parts[1]))
                    return -1; // t1 always before t2
                else
                    return t1Parts[0].compareTo(t2Parts[1]);
            }

            if(t1Parts.length == 3 && t2Parts.length == 1){
                if(t1Parts[1].equals(t2Parts[0]))
                    return 1; // t1 always after t2
                else
                    return t1Parts[1].compareTo(t2Parts[0]);
            }
    ...
    public static class ReducerClass extends Reducer<Text, Text, Text, Text> {
        ...
        public void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException {
            ...
            if (keyParts.length != 3) {
                ...
                valueOut.set("N3=" + lastValueString + " C1=" + lastValue);
            }
    ...
    public static void main(String[] args) throws Exception {
        ...
        // Initialize the Job
        Job job = Job.getInstance(conf, "C1 Step");
        job.setJarByClass(C1Step.class);

        // Setup Input
        FileInputFormat.addInputPath(job, new Path("s3://dsp2/output/word-count"));
        job.addCacheFile(new Path("s3://dsp2/" + LOG4J_FILE).toUri());

        // Setup Mapper
        job.setMapperClass(C1Step.MapperClass.class);
        job.setMapOutputKeyClass(Text.class);
        job.setMapOutputValueClass(Text.class);

        // Setup Partitioner
        job.setPartitionerClass(C1Step.PartitionerClass.class);

        // Setup Comparator
        job.setSortComparatorClass(C1Step.ComparatorClass.class);
        job.setGroupingComparatorClass(Text.Comparator.class);

        // Setup Reducer
        job.setReducerClass(C1Step.ReducerClass.class);
        job.setOutputKeyClass(Text.class);
        job.setOutputValueClass(Text.class);

        // Setup Output
        FileOutputFormat.setOutputPath(job, new Path("s3://dsp2/output/c1-output"));
